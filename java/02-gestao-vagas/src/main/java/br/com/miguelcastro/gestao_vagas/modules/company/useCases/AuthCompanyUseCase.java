...
public class AuthCompanyUseCase {
	...
	public AuthCompanyResponseDTO execute(AuthCompanyRequestDTO authCompanyRequestDTO) {
		...
		var token = JWT.create().withIssuer("javagas").withSubject(company.getId().toString())
				.withClaim("roles", Arrays.asList("COMPANY")).withExpiresAt(expiresIn).sign(algorithm);

		var formattedExpireDate = LocalDateTime.ofInstant(expiresIn, ZoneId.systemDefault());
		var authCompanyResponse = AuthCompanyResponseDTO.builder().access_token(token).expires_in(formattedExpireDate)
				.build();
